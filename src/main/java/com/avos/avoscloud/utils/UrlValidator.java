 ...
 * fragments are flagged as illegal.</li>
 * <li>ALLOW_ALL_SCHEMES - [FALSE] By default only http, https, and ftp are considered valid
 * schemes. Enabling this option will let any scheme pass validation.</li>
 * </ul>
 *
 * <p>
...
public class UrlValidator implements Serializable {
  ...
  /**
   * Allow local URLs, such as http://localhost/ or http://machine/ . This enables a broad-brush
   * check, for complex local machine name validation requirements you should create your validator
   * with a {@link RegexValidator} instead ({@link #UrlValidator(RegexValidator, long)})
   */
  ...
  private static final String AUTHORITY_CHARS_REGEX = "\\p{Alnum}\\-\\."; // allows for IPV4 but not
                                                                          // IPV6
  private static final String IPV6_REGEX = "[0-9a-fA-F:]+"; // do this as separate match because :
                                                            // could cause ambiguity with port
                                                            // prefix
  ...
  private static final String USERINFO_FIELD_REGEX = USERINFO_CHARS_REGEX + "+:" + // At least one
                                                                                   // character for
                                                                                   // the name
      USERINFO_CHARS_REGEX + "*@"; // password may be absent
  private static final String AUTHORITY_REGEX = "(?:\\[(" + IPV6_REGEX + ")\\]|(?:(?:"
      + USERINFO_FIELD_REGEX + ")?([" + AUTHORITY_CHARS_REGEX + "]*)))(:\\d*)?(.*)?";
  ...
  /**
   * Regular expressions used to manually validate authorities if IANA domain name validation isn't
   * desired.
   */
   ...
   * 
   * @param options The options should be set using the public constants declared in this class. To
   *        set multiple options you simply add them together. For example, ALLOW_2_SLASHES +
   ...
   * Initialize a UrlValidator with the given validation options.
   * 
   * @param authorityValidator Regular expression validator used to validate the authority part This
   *        allows the user to override the standard set of domains.
   * @param options Validation options. Set using the public constants of this class. To set
   *        multiple options, simply add them together:
   *        <p>
  ...
  /**
   * Validate scheme. If schemes[] was initialized to a non null, then only those schemes are
   * allowed. Otherwise the default schemes are "http", "https", "ftp". Matching is case-blind.
   * 
  ...
  /**
   * Tests whether the given flag is on. If the flag is not a power of 2 (ie. 3) this tests whether
   * the combination of flags is on.
   *
  ...
  /**
   * Tests whether the given flag is off. If the flag is not a power of 2 (ie. 3) this tests whether
   * the combination of flags is off.
   *
   * @param flag Flag value to check.
   *
